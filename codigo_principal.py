...
num_players=1
personajes_elegidos=[]
personajes_no_elegidos=[]
personajes_disponibles = [
    {"nombre": "Franco Colapinto", "imagen": "autos/Williams_car.png"},
    {"nombre": "Max Verstappen", "imagen": "autos/Redbull_car.png"},
    {"nombre": "Charles Leclerc", "imagen": "autos/Ferrari_car.png"},
    {"nombre": "Lando Norris", "imagen": "autos/Mclaren_car.png"},
    {"nombre": "Rayo Mcqueen 10", "imagen": "autos/cars.png"},]
pygame.init()
...
def renderizar_texto_con_contorno(texto, fuente, color_texto, color_contorno, pos):
    ...
    for x in (-2, 2):
        for y in (-2, 2):
            screen.blit(fuente.render(texto, True, color_contorno), (rect.x + x, rect.y + y))
...
def pantalla_eleccion_personaje(num_players):
    ...
    global personajes_elegidos, personajes_no_elegidos
    # Cargar la imagen de fondo
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
            if event.type == pygame.KEYDOWN:
                # Si eligen a un personaje, sale su pantalla de carga y se grega a la lista de personajes elegidos
                if event.key == pygame.K_w:
                    personajes_elegidos.append({"nombre": "Franco Colapinto","imagen": "autos/Williams_car.png"})
                    #personajes_no_elegidos.append({})
                    pantalla_de_carga_personaje('pantalla_personajes/colapinto.png', (173, 216, 230), (0, 0, 128), 'Williams')
                    players_selected += 1
                elif event.key == pygame.K_r: 
                    personajes_elegidos.append({"nombre": "Max Verstappen","imagen": "autos/Redbull_car.png"})
                    pantalla_de_carga_personaje('pantalla_personajes/red bull.png', (255, 0, 0), (0, 0, 128), 'Red Bull')
                    players_selected += 1
                elif event.key == pygame.K_f:  
                    personajes_elegidos.append({'nombre': "Charles Leclerc",'imagen':"autos/Ferrari_car.png"})
                    pantalla_de_carga_personaje('pantalla_personajes/ferrari.png', (255, 20, 0), (255, 24, 7), 'Ferrari')
                    players_selected += 1
                elif event.key == pygame.K_m:
                    personajes_elegidos.append({'nombre':"Lando norris",'imagen':'autos/Mclaren_car.png'})
                    pantalla_de_carga_personaje('pantalla_personajes/mclaren.png', (254, 80, 0), (255, 200, 0), 'McLaren')
                    players_selected += 1
                elif event.key == pygame.K_c:
                    personajes_elegidos.append({'nombre':"El Rasho Mcqueen",'imagen':'autos/cars.png'})
                    pantalla_de_carga_personaje('pantalla_personajes/mclaren.png', (254, 80, 0), (255, 200, 0), 'McLaren')
                    players_selected += 1
...
def draw_controls_message(screen):
    ...
    text_rect = text.get_rect(bottomleft=(300, screen.get_height() - 10))
    screen.blit(text, text_rect)
# Crear una instancia de Track
def draw_lap_counter(screen, car, track):
    font = pygame.font.Font(None, 36)
    lap_message = f"Vueltas: {car.lap_count}/3"
...
personaje_random=random.choice(personajes_no_elegidos)
auto_car = AutoCar(personaje_random['nombre'], personaje_random['imagen'])
...
while running:
    ...
    if num_players==2:
        if player_car.lap_count ==3:
            winner_declared = True
            winner_text = f"{personaje_jugador_1['nombre']} gana la ronda"
            player_car.set_speed(0)
            auto_car.set_speed(0)
            player_car2.set_speed(0)
        elif player_car2.lap_count ==3 :
            winner_declared = True
            winner_text = f"{personaje_jugador_2['nombre']} gana la ronda"
            player_car.set_speed(0)
            auto_car.set_speed(0)
            player_car2.set_speed(0)
        elif auto_car.lap_count == 3:
            winner_declared = True
            winner_text = f"{personaje_random['nombre']} gana la ronda"
            player_car.set_speed(0)
            auto_car.set_speed(0)
            player_car2.set_speed(0)
    else:
