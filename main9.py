...
num_players=1
cars_config = {
    pygame.K_w: {"name": "Franco Colapinto", "image": "autos/Williams_car.png"},
    pygame.K_m: {"name": "Lando Norris", "image": "autos/Mclaren_car.png"},
    pygame.K_r: {"name": "Max Verstappen", "image": "autos/Redbull_car.png"},
    pygame.K_f: {"name": "Charles Leclerc", "image": "autos/Ferrari_car.png"},
}
# Inicializar Pygame
pygame.init()
...
def pantalla_eleccion_personaje(num_players):
    ...
    global player_car  # Necesitamos acceder y definir `player_car` globalmente
    spawn_point = [400, 300]  # Ejemplo de punto de inicio, ajusta según necesidad
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                sys.exit()
            if event.type == pygame.KEYDOWN:
                # Si se presiona una de las teclas definidas en `cars_config`
                if event.key in cars_config:
                    # Obtener la configuración correspondiente del diccionario
                    car_config = cars_config[event.key]
                    player_car = PlayerCar(
                        car_config["name"],1,[pygame.K_UP, pygame.K_DOWN, pygame.K_LEFT, pygame.K_RIGHT],spawn_point,car_config["image"]
                    )
                    players_selected += 1
...
def main():
    ...
    if num_players == 1:
        ...
        pantalla_eleccion_personaje(2)w
    pygame.quit()
