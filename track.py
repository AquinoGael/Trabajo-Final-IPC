...
#funcion track del archivo template, pero añadido una funcion para ver la pista
import numpy as np
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Pista generada con Pygame")
    ...
    # Crear una instancia de la pista
    track = Track(x_max=screen_width, y_max=screen_height)

    # Convertir puntos de la pista a formato Pygame
    middle_of_track_points = [(int(x), int(y)) for x, y in track.middle_of_track]

    # Dibujar la pista en la pantalla
    running = True
    while running:
        # Manejar eventos
        ...
        pygame.draw.lines(screen, black, True, middle_of_track_points, 2)

        # Dibujar los bordes de la pista
        if hasattr(track.track_polygon, "exterior"):
            exterior_coords = [(int(x), int(y)) for x, y in track.track_polygon.exterior.coords]
            pygame.draw.polygon(screen, red, exterior_coords, 2)
